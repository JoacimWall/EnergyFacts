"""Config flow for EnergyFacts integration - 5-step setup."""
...
class EnergyFactsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for EnergyFacts."""
    ...
    async def async_step_investment(
        ...
        if user_input is not None:
            ...
            title = self._data.get(CONF_HOME_NAME, "EnergyFacts")
    ...
    ) -> EnergyFactsOptionsFlow:
        """Get the options flow handler."""
        return EnergyFactsOptionsFlow(config_entry)


class EnergyFactsOptionsFlow(config_entries.OptionsFlow):
    """Handle options flow for EnergyFacts (edit steps 3-5 + yearly params)."""
