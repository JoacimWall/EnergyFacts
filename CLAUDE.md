...
Home Assistant custom integration ("EnergyFacts") that tracks solar energy production economics using the Tibber API, calculates financial metrics, and projects ROI over 30 years. Includes a custom Lovelace card for ROI visualization. Domain: `energy_facts`.
...
npm run build       # build with rollup → energy-facts-roi-card.js
npm run watch       # dev mode with file watching
```

The compiled JS is copied to `custom_components/energy_facts/energy-facts-roi-card.js` for HA to serve — never forget this.

### Panel (TypeScript)
```bash
cd custom_cards/energy-facts-panel
npm install         # one-time setup
npm run build       # build with rollup → energy-facts-panel.js
```

After building, always copy the built JS to the serving directory:
```bash
cp custom_cards/energy-facts-panel/energy-facts-panel.js custom_components/energy_facts/energy-facts-panel.js
```
...
### Key Modules (in `custom_components/energy_facts/`)
...
- **entity.py**: Base `EnergyFactsEntity(CoordinatorEntity)` — sets `unique_id`, `device_info`, and `has_entity_name`.

### Lovelace Card (`custom_cards/energy-facts-roi-card/`)
...
### Panel (`custom_cards/energy-facts-panel/`)
