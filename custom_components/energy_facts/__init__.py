...
from .coordinator import EnergyFactsCoordinator
from .database import EnergyFactsDatabase
...
CARD_JS = "energy-facts-roi-card.js"
PANEL_JS = "energy-facts-panel.js"
CARD_URL = f"/{DOMAIN}/{CARD_JS}?v={_VERSION}"
PANEL_URL = f"/{DOMAIN}/{PANEL_JS}?v={_VERSION}"
CARD_DIR = Path(__file__).parent


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up EnergyFacts from a config entry."""
    ...
    database = EnergyFactsDatabase(hass, entry.entry_id)
    ...
    # Create coordinator
    coordinator = EnergyFactsCoordinator(
        hass=hass,
        session=session,
        storage=database,
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload a EnergyFacts config entry."""
    ...
    if unload_ok:
        ...
        if not hass.data.get(DOMAIN):
            try:
                hass.components.frontend.async_remove_panel("energy-facts")
...
async def async_remove_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    _LOGGER.warning("Removing EnergyFacts: cleaning up all data")
    ...
    _LOGGER.warning("EnergyFacts cleanup complete")
...
async def _register_panel(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    try:
        ...
        if "energy-facts" in hass.data.get("frontend_panels", {}):
            hass.components.frontend.async_remove_panel("energy-facts")

        await async_register_panel(
            hass,
            frontend_url_path="energy-facts",
            webcomponent_name="energy-facts-panel",
